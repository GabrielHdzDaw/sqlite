...
-- 102. Añade un nuevo producto en la tabla "productos", con código 12, nombre "SSD P3 Plus 2 TB", precio desconocido
-- (pero no debes usar la palabra NULL), del fabricante 6. Comprueba si aparece en la vista y si aparece en la nueva tabla.
INSERT INTO productos (id, nombre, id_fabricante)
VALUES (12, 'SSD P3 Plus 2 TB', 6);
--Aparece en la vista pero no en la tabla


-- 103. Añade a la tabla productosYFabricantes un nuevo campo llamado "comentario", de tipo TEXT.
ALTER TABLE productosYFabricantes ADD (comentario TEXT(100));

-- 104. Añade a la tabla la restricción de que el código de producto actúe como clave primaria.
ALTER TABLE productosYFabricantes ADD CONSTRAINT pk_productosYFabricantes PRIMARY KEY (id);

-- 105. Renombra la tabla "productosYFabricantes" para que pase a llamarse "productosFabricantes".
ALTER TABLE productosYFabricantes RENAME TO productosFabricantes;

-- 106. Renombra el campo "comentario" de "productosFabricantes" para que pase a llamarse "comentarios".
ALTER TABLE productosFabricantes RENAME COLUMN comentario TO comentarios;
...
SELECT nombre, precio FROM productos ORDER BY precio DESC FETCH FIRST ROW ONLY;
...
SELECT nombre FROM productos WHERE nombre LIKE '_o%';
SELECT nombre FROM productos WHERE SUBSTR(nombre, 2, 1) = 'o';
SELECT nombre FROM productos WHERE INSTR(nombre, 'o', 2) = 2;

-- 121. Modifica el producto de código 12 para indicar que su precio es 125.
UPDATE productos SET precio = 125
WHERE id = 12;
...
WHERE EXISTS (
    SELECT * FROM productos p2
    WHERE SUBSTR(p1.nombre, 1, 7) = SUBSTR(p2.nombre, 1, 7)
    AND p1.id <> p2.id
);
